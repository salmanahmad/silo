...
public class MonitorEnter implements Expression {

    Node node;

    public MonitorEnter(Node node) {
        this.node = node;
    }
    ...
    public void emit(CompilationContext context) {
        ...
        if(node.getChildren().size() != 1) {
            throw new RuntimeException("monitorenter must have 1 argument");
        }
        ...
        if(expression.type(context).isPrimitive()) {
            throw new RuntimeException("monitorenter cannot be applied to primitive values.");
        }

        expression.emit(context);

        generator.monitorEnter();
